...
export class MarkComponent implements OnInit {
  ...
  eleveDevoirsGrouped: EleveDevoirDto[][] = []; // Déclarez la propriété pour stocker les données groupées
  ...
  onPeriodeChange(event: Event): void {
    ...
    if (this.periodeId) {
      this.devoirs = []; // Reset devoirs
      this.eleveDevoirs = []; // Reset eleveDevoirs
      this.eleveDevoirsGrouped = []; // Reset grouped data
  ...
  onDevoirChange(event: any): void {
    ...
    if (selectedValue === '*') {
      this.selectedDevoirId = null; // Reset selected devoir ID
      this.loadAllDevoirsForStudents(); // Load all devoirs for students
    } else {
      this.selectedDevoirId = parseInt(selectedValue);
      this.eleveDevoirs = []; // Clear the list before loading new data
      this.loadDevoirsEleves(); // Load the assignments for the selected devoir
    }
  }

  displayAllDevoirNames(): void {
    if (this.devoirs.length > 0) {
      console.log('Liste des devoirs:');
      this.devoirs.forEach((devoir) => {
        console.log(devoir.libelle); // Log each devoir's name
      });
    } else {
      console.log('Aucun devoir disponible.');
    }
  }
  ...
  groupDevoirsByPeriode(): void {
    if (this.periodeId) {
      const groupedByStudent = this.eleveDevoirs.reduce((acc, eleveDevoir) => {
        const periodeId = eleveDevoir.devoir?.periode?.idPeriode;
        if (periodeId === this.periodeId) {
          const eleveId = eleveDevoir.eleve?.idEleve;
          if (eleveId) {
            if (!acc[eleveId]) {
              acc[eleveId] = [];
            }
            acc[eleveId].push(eleveDevoir);
          }
        }
        return acc;
      }, {} as Record<number, EleveDevoirDto[]>);

      // Convertir l'objet en tableau de tableaux
      this.eleveDevoirsGrouped = Object.values(groupedByStudent);

      // Trier les groupes par numéro d'élève
      this.eleveDevoirsGrouped.sort(
        (a, b) => (a[0].eleve?.num ?? 0) - (b[0].eleve?.num ?? 0)
      );
    }
  }
  ...
  trackByEleveDevoirs(index: number, item: EleveDevoirDto[]): number {
    return item[0]?.eleve?.idEleve ?? index; // Utilisez l'ID de l'élève pour le suivi
  }

  trackByEleveDevoir(index: number, item: EleveDevoirDto): number {
    return item.idEleveDevoir ?? index; // Utilisez l'ID du devoir pour le suivi
  }
  ...
  onNoteChange(event: Event, eleveDevoir: EleveDevoirDto): void {
    ...
    eleveDevoir.note = newValue ? parseFloat(newValue) : null;
  }
  ...
  imprimer() {
    ...
    img.onload = () => {
      ...
      // Texte en arabe
      const etab = `${this.etablissement?.libelle || 'Établissement Non Défini'}`;
      const prof = `${this.professeur?.nom || 'Nom Non Défini'} ${this.professeur?.prenom || 'Prénom Non Défini'}`;
      ...
      if (this.selectedDevoirId === null) {
        // Imprimer toutes les données si l'option * est sélectionnée
        const anneeScolaire = this.anneeScolaire?.libelle || 'Année Scolaire Non Défini';
        const periode = this.periodes.find(p => p.idPeriode === this.periodeId)?.libelle || 'Période Non Défini';
        const classe_nom = this.classeEleves[0].classe?.libelle || 'Classe Non Défini';
        (doc as any).autoTable({
          startY: textY + 5,
          head: [],
          body: [
        ...
        // Ajouter le titre "نقط المراقبة المستمرة" au milieu
        const titleText = "نقط المراقبة المستمرة";
        const titleWidth = doc.getTextWidth(titleText);
        const titleX = (doc.internal.pageSize.getWidth() - titleWidth) / 2;
        const titleY = doc.autoTable.previous.finalY + 10;
        doc.text(titleText, titleX, titleY);

        const tableRows = this.eleveDevoirsGrouped.map((group) => {
          const student = group[0].eleve;
          const notes = this.devoirs.map((devoir) => {
            const eleveDevoir = group.find((ed) => ed.devoir?.idDevoir === devoir.idDevoir);
            return eleveDevoir ? eleveDevoir.note : 'N/A';
          });
          const moyenne = this.calculateFinalNoteForStudent(group);
          return [student?.num, student?.nom, ...notes, moyenne, ''];
        });

        const tableColumns = [['Numéro', 'Nom', ...this.devoirs.map(d => d.libelle), 'معدل الدورة', 'معدل']];
        ...
        // Calculer les statistiques basées sur la colonne de moyenne
        const moyennes = tableRows.map(row => row[row.length - 2] as number);
        const meilleureMoyenne = Math.max(...moyennes);
        const pireMoyenne = Math.min(...moyennes);
        const nombreNotesInf10 = moyennes.filter(m => m < 10).length;
        const nombreNotesEntre10et12 = moyennes.filter(m => m >= 10 && m < 12).length;
        const nombreNotesSup14 = moyennes.filter(m => m > 14).length;

        // Trouver les élèves avec la meilleure et la pire moyenne
        const eleveMeilleureMoyenne = tableRows.find(row => row[row.length - 2] === meilleureMoyenne)?.[1] || 'Inconnu';
        const elevePireMoyenne = tableRows.find(row => row[row.length - 2] === pireMoyenne)?.[1] || 'Inconnu';

        const statsColumns = [['Meilleure Moyenne', 'Élève', 'Pire Moyenne', 'Élève', 'Notes < 10', 'Notes 10-12', 'Notes > 14']];
        const statsRows = [[meilleureMoyenne, eleveMeilleureMoyenne, pireMoyenne, elevePireMoyenne, nombreNotesInf10, nombreNotesEntre10et12, nombreNotesSup14]];

        let statsTableY = (doc as any).autoTable.previous.finalY + 10;
        (doc as any).autoTable({
          startY: statsTableY,
          head: statsColumns,
          body: statsRows,
        ...
        const sanitizeFileName = (str: string) =>
          str.replace(/[^a-zA-Z0-9\u0600-\u06FF\s]/g, '').replace(/\s+/g, '_');

        const nomFichier = `${sanitizeFileName(periode)}_${sanitizeFileName(this.classeEleves[0].classe?.libelle!)}.pdf`;

        doc.save(nomFichier);
      } else {
        // Logique existante pour un devoir spécifique
        this.getDevoirById(this.selectedDevoirId).subscribe(
          () => {
            ...
            const tableRows = this.eleveDevoirs.map((eleveDevoir: EleveDevoirDto) => {
              return {
                ...
                note: eleveDevoir.note !== undefined ? eleveDevoir.note : 'N/A',
              };
            });

            if (tableRows.length === 0) {
              console.warn("Aucune donnée d'élève trouvée, utilisant des données de test.");
              tableRows.push(
            ...
            (doc as any).autoTable({
              startY: tableY,
              head: tableColumns,
              body: tableRows.map((row) => [row.num, row.code, row.nom, row.note]),
              styles: {
                font: 'Amiri',
                fontSize: 11,
                cellPadding: 1,
            ...
            const notes = tableRows.map((row) => row.note).filter((note) => note !== 'N/A') as number[];
            const meilleureNote = Math.max(...notes);
            const pireNote = Math.min(...notes);
            const moyenneNote = notes.reduce((a, b) => a + b, 0) / notes.length;

            const statsColumns = [['Meilleure Note', 'Pire Note', 'Moyenne de la Classe']];
            const statsRows = [[meilleureNote, pireNote, moyenneNote.toFixed(2)]];
            ...
            const nomFichier = `${sanitizeFileName(periode)}_${sanitizeFileName(classeNom!)}.pdf`;
  ...
  getNoteForDevoir(
    ...
    const foundDevoir = this.eleveDevoirs.find(
      (ed) =>
        ed.eleve?.idEleve === eleveDevoir.eleve?.idEleve &&
        ed.devoir?.idDevoir === devoir.idDevoir
    );
    return foundDevoir && foundDevoir.note != null ? foundDevoir.note : '';
  }

  loadAllDevoirsForStudents(): void {
    const marksArray = this.marks;
    marksArray.clear(); // Réinitialiser le FormArray
    this.eleveDevoirs = []; // Vider les données existantes pour éviter la duplication

    if (this.selectedClasseId !== null) {
      // Récupérer les élèves en fonction de la classe sélectionnée
      this.classeEleveService.findByClasseId(this.selectedClasseId).subscribe(
        (classeEleves: ClasseEleveDto[]) => {
          const elevesInClasse = classeEleves
            .map((classeEleve) => classeEleve.eleve?.idEleve)
            .filter((eleveId) => eleveId !== undefined);

          // Créer un tableau d'observables pour chaque élève
          const observables = elevesInClasse.map((eleveId) =>
            this.elevedevoirService.findAllByEleve(eleveId!)
          );

          // Utiliser forkJoin pour attendre que tous les observables soient complétés
          forkJoin(observables).subscribe(
            (results: EleveDevoirDto[][]) => {
              // Aplatir les résultats et assigner à eleveDevoirs
              this.eleveDevoirs = results.flat();

              // Grouper les devoirs par élève
              const groupedByStudent = this.eleveDevoirs.reduce(
                (acc, eleveDevoir) => {
                  const eleveId = eleveDevoir.eleve?.idEleve;
                  if (eleveId) {
                    if (!acc[eleveId]) {
              ...
              // Remplir le tableau de formulaire avec les données récupérées
              this.populateMarkArray();
            },
            (error) => {
              console.error(
  ...
  getPaginatedEleveDevoirs(): EleveDevoirDto[][] {
    const startIndex = (this.currentPage - 1) * this.itemsPerPage;
    const endIndex = startIndex + this.itemsPerPage;
    return this.eleveDevoirsGrouped.slice(startIndex, endIndex);
  }

  getNoteFinale(eleveDevoir: EleveDevoirDto, devoir: DevoirDto): number {
    const note = eleveDevoir.note;
    const coefficient = devoir.coefficient! / 100 ?? 1; // Use 1 as default if coefficient is undefined
    console.log(coefficient);
    return note !== null ? note! * coefficient : 0;
  }

  calculateFinalNoteForStudent(eleveDevoirs: EleveDevoirDto[]): number {
    if (this.periodeId) {
      // Filtrer les devoirs pour ne garder que ceux de la période sélectionnée
      const devoirsForCurrentPeriod = eleveDevoirs.filter(
        (eleveDevoir) => eleveDevoir.devoir?.periode?.idPeriode === this.periodeId
      );

      if (devoirsForCurrentPeriod.length === 0) {
        return 0; // Return 0 if there are no assignments for the current period
      }

      let totalWeightedNotes = 0;
      let totalCoefficients = 0;

      devoirsForCurrentPeriod.forEach((eleveDevoir) => {
        const note = eleveDevoir.note ?? 0;
        const coefficient = eleveDevoir.devoir?.coefficient ?? 1;
        totalWeightedNotes += note * coefficient;
        totalCoefficients += coefficient;
      });

      const finalNote = totalWeightedNotes / totalCoefficients;
      return parseFloat(finalNote.toFixed(2)); // Return the final note with two decimal places
    }
    return 0; // Return 0 if no period is selected
  }

  // Example usage
  getFinalNoteForStudent(eleveDevoir: EleveDevoirDto): number {
    const studentDevoirs = this.eleveDevoirs.filter(
      (ed) => ed.eleve?.idEleve === eleveDevoir.eleve?.idEleve
    );
    return this.calculateFinalNoteForStudent(studentDevoirs);
  }
}
