...
export class MarkComponent implements OnInit {
  ...
  imprimer() {
    ...
    img.onload = () => {
      ...
      if (this.selectedDevoirId === null) {
        ...
        const moyennes = tableRows.map(row => row[row.length - 3] as number);
        ...
        const eleveMeilleureMoyenne = tableRows.find(row => row[row.length - 3] === meilleureMoyenne)?.[1] || 'Inconnu';
        const elevePireMoyenne = tableRows.find(row => row[row.length - 3] === pireMoyenne)?.[1] || 'Inconnu';
  ...
  // nextPage(): void {
  //   if ((this.currentPage * this.itemsPerPage) < this.eleveDevoirsGrouped.length) {
  //     this.currentPage++;
  //   }
  // }
